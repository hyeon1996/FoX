...
class FOX_QMIX:
    ...
    def train_predict(self, batch: EpisodeBatch, t_env: int):
        ...
        random_batch = [random.randrange(batch.batch_size) for _ in range(25)]
        random_time = [random.randrange(batch.max_seq_length - 1) for _ in range(25)]
        random_agent = [random.randrange(self.args.n_agents) for _ in range(25)]
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, show_demo=False, save_data=None):
        ...
        with th.no_grad():
            ...
            visit_reward = th.where(visit != 0, self.args.beta1 / th.sqrt(visit), th.zeros_like(visit))

            non_zero_mask = visit_reward != 0
            non_zero_values = visit_reward[non_zero_mask]
            mean_non_zero = th.mean(non_zero_values)
            ...
            for agent in range(self.args.n_agents):
                ...
                ir = ((ir - min_ir) / (max_ir - min_ir)) * self.normalizer
        ...
        targets = rewards + diversity_reward + visit_reward + self.args.gamma * (1 - terminated) * target_max_qvals

        non_zero = visit_reward[visit_reward != 0]
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            self.logger.log_stat("visit_reward", mean_non_zero, t_env)
